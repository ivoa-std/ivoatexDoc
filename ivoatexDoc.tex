...
A container file for this purpose is maintained in the \ivoatex GitHub repository.
In the simplest case -- when pre-built-images are available -- the workflow then is like this:
...
$ podman run --rm --tty --interactive \
    ...
    ghcr.io/ivoa-std/ivoadoc:latest
...
There are pre-built containers for the \verb|linux/amd64| and \verb|linux/arm64| architectures,
which should allow the container to be run on most modern computers running Linux, MacOS and Windows (via WSL).
If there is no pre-built container for your architecture, create an issue in the \ivoatex GitHub repository
with a request for the new architecture to be added.

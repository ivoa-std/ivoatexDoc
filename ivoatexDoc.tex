...
IVOA standards are required to be available in both HTML and PDF
formats.  Many come with further artefacts, such as XML schema files or
perhaps OpenAPI specifications.  To make the document repository create
links to these files, uploads must contain a manifest declaring what to
publish on the standard's landing page.  \ivoatex~generates this
manifest from a file called \verb|MANIFEST.in| at upload time.
\ivoatex's \verb|make-templates.sh| will install a default for it
containing:

\lstinputlisting[basicstyle=\footnotesize]{ivoatex/MANIFEST.in}

This is enough when all you have is text.  To add more links, add
semicolon-separated triples of label, doctype, and a repository-relative path.

Doctype has to come from a controlled vocabulary that a this point is

\begin{description}
\item[document] A rendering of the standard itself; this should in
general be restricted to the HTML and PDF renderings in exactly the form
as generated; there is templating involved because the standard file
names are not constant.

\item[schema] This is strictly reserved for XML schema files that should
end up in the IVOA XML schema repository.

\item[example] This is for bundled files that may be useful in
implementations and are useful stand-alone.  For instance, sample
VOTable files to test a parser on could be included in this way.  There
expectation that standards link all files of this type coming with them
on the landing page; an alternative or complement is in-document links
using auxiliaryurl (cf.~\ref{sect:links}).

\end{description}

Editors needing other document types are requested to file issues
against \ivoatex's github repository, citing the specific standard that
needs them.

To migrate pre-1.7\ \ivoatex documents, do
\begin{lstlisting}
make update
cp ivoatex/MANIFEST.in .
git add MANIFEST.in
echo MANIFEST >> .gitignore
\end{lstlisting}
Then inspect the \verb|AUX_FILES| variable in the make file to see if
additional entries should be added to \verb|MANIFEST.in|.

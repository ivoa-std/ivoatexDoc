...
To display such a figure, first add

\begin{lstlisting}
\usepackage{tikz}
\end{lstlisting}

\noindent in your document's preamble.  Then write
\verb|\tikzfigure{some-diagram}| where the figure is supposed to show
up.

In the PDF-rendering branch, this is not much more than an
\verb|include|, whereas in HTML, this will lead to a reference to an SVG
called \verb|some-diagram.tikz.svg|.  To make ivoatex build this SVG,
...
In this way \verb|\tikzfigure{hello}| yields: \tikzfigure{hello}.

Keep
the \verb|.tikz.tex| file under version control.  As ivoatexDoc itself
does, you should not put the generated SVG under version control unless
it is paramount to avoid pdf2svg as a build dependency.  The diffs on
that SVG will be ugly, confusing, and useless.

To develop such figures, it is probably most convenient to run
\verb|make hello.tikz.pdf| and inspect the resulting PDF.
